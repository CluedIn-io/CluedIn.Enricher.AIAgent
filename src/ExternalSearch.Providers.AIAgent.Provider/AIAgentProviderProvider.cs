...
using CluedIn.ExternalSearch.Providers.AIAgent;
using CluedIn.Providers.Models;
using Constants = CluedIn.ExternalSearch.Providers.AIAgent.Constants;

namespace CluedIn.Provider.ExternalSearch.AIAgent
{
    public class AIAgentProviderProvider : ProviderBase, IExtendedProviderMetadata, IExternalSearchProviderProvider
    {
        public IExternalSearchProvider ExternalSearchProvider { get; }

        public AIAgentProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(appContext, GetMetaData())
        {
        ...
        public override async Task<CrawlJobData> GetCrawlJobData(ProviderUpdateContext context, IDictionary<string, object> configuration, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var result = new AIAgentExternalSearchJobData(configuration);
        ...
        {
            if (jobData is AIAgentExternalSearchJobData result)
            {
                return await Task.FromResult(result.ToDictionary());
            }

            throw new InvalidOperationException($"Unexpected data type for {nameof(AIAgentExternalSearchJobData)}, {jobData.GetType()}");
        }
